...
This skill is vendored under the MIT license, copyright Melodic Software. The
license is stored beside this file and must stay with any copy. It owns
`docs/ARCHITECTURE.md` together with `c4-model`, as the project file set in
`buzz-team-communication` states. The only change from the original is the
step list below, which no longer requires a `docs-management` skill or a
Perplexity MCP server; neither ships with this plugin.
...
1. **Read the project's existing architecture documentation** before writing,
   so the result continues it instead of replacing it
2. **Check the current arc42 version** at [`arc42.org`](https://arc42.org) when
   a web lookup is available, and say so when it is not
3. **Base guidance on the official arc42 template**
